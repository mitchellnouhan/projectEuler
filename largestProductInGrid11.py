...
#iterate thru the grid using two indexes (i,j) -> (row, col) zero-indexed
gridLen = len(grid)
lol = 4
maxProd = 1
for row in range(gridLen):
    for col in range(gridLen):
        #check north, south, west, east, and all 4 diagnals, update maxProduct
        north = row >= (lol - 1)
        south = row  <= ((gridLen - 1) - (lol - 1))
        east = col <= ((gridLen - 1) - (lol - 1))
        west =  col >= (lol - 1)
